<template>
  <div class="knowledge-graph-container">
    ...
    <div class="control-panel">
      ...
      <el-card class="filter-card">
        <div class="filter-controls">
          ...
          <el-select v-model="viewMode" placeholder="完整图谱" style="width: 180px;" @change="handleViewModeChange">
    ...
    <el-card class="graph-card" v-loading="loading">
      <div id="knowledge-graph" ref="graphContainer" style="width: 100%; height: 800px;"></div>
    </el-card>
  ...
  setup() {
    ...
    const loadGraphData = async () => {
      ...
      try {
        ...
        if (graphData.nodes.length === 0) {
          ...
          graphData.nodes = [
            { id: 'rm_1', name: '聚醚多元醇', type: 'raw_material', code: 'RM001', data: { supplier: '示例供应商' } },
            { id: 'rm_2', name: '异氰酸酯', type: 'raw_material', code: 'RM002', data: { supplier: '示例供应商2' } },
            { id: 'int_1', name: '预聚体', type: 'intermediate', code: 'INT001', data: { intermediate_type: '预聚体' } },
            { id: 'formula_1', name: '配方A', type: 'formula', code: 'F001', data: { version: '1.0' } },
            { id: 'perf_1', name: '性能测试1', type: 'performance', code: 'P001', data: { rating: 4.5 } }
          ]
          graphData.edges = [
        ...
        if (graphData.nodes.length > 0) {
          ...
          // 初始化过滤数据
          filterData()
          ElMessage.success(`知识图谱加载成功：${graphData.nodes.length}个节点，${graphData.edges.length}条边`)
        } else {
    ...
    const renderGraph = () => {
      ...
      if (viewMode.value !== 'full') {
        ...
        console.log(`过滤后节点: ${filteredNodesData.length}, 边: ${filteredEdgesData.length}`)
      }

      // 计算节点大小（基于连接数）
      const getNodeSize = (nodeId, nodeType) => {
        const connections = filteredEdgesData.filter(e => e.source === nodeId || e.target === nodeId).length
        const baseSize = {
          raw_material: 35,
          intermediate: 40,
          formula: 50,
          ...
          formatter: (params) => {
            if (params.dataType === 'node') {
              ...
              let html = `<div style="padding: 10px;">
                <h4 style="margin: 0 0 10px 0; color: ${nodeTypeConfig[node.type]?.color};">
                  ${node.name}
                </h4>
                <p style="margin: 5px 0;"><strong>类型:</strong> ${getNodeTypeLabel(node.type)}</p>
                <p style="margin: 5px 0;"><strong>编号:</strong> ${node.code || '-'}</p>`
              
              // 根据节点类型显示额外信息
              if (node.type === 'raw_material' && node.data) {
                html += `<p style="margin: 5px 0;"><strong>供应商:</strong> ${node.data.supplier || '-'}</p>`
              } else if (node.type === 'intermediate' && node.data) {
                html += `<p style="margin: 5px 0;"><strong>固含量:</strong> ${node.data.solid_content || '-'}%</p>`
              } else if (node.type === 'formula' && node.data) {
                html += `<p style="margin: 5px 0;"><strong>版本:</strong> ${node.data.version || '-'}</p>`
              } else if (node.type === 'performance' && node.data) {
                html += `<p style="margin: 5px 0;"><strong>评分:</strong> ${node.data.rating || '-'}/5</p>`
              }
