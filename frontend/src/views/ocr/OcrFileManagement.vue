      ...
      <div class="toolbar">
        ...
        <div class="actions">
          ...
          <div class="search-box">
            ...
            />
            <button class="btn" :disabled="loading" @click="onSearch">
              搜索
            </button>
            <button
              class="btn"
          ...
          <span class="filter-indicator"
            >当前筛选：{{ selectedDocumentTypeLabel }}</span
          >
          <button class="btn" @click="router.push('/ocr-center')">
            返回OCR中心
          </button>
          ...
          <button class="btn primary" @click="router.push('/ocr/upload')">
            上传文件
          </button>
        </div>
      </div>
        ...
        <div class="pagination-bar">
          ...
          <div class="pagination-actions">
            ...
            <div class="page-jump">
              ...
              />
              <span>页</span>
              <button class="page-btn" :disabled="loading" @click="jumpToPage">
                跳转
              </button>
            </div>
          </div>
                ...
                <td class="ops-cell">
                  <div class="action-group">
                    <button
                      ...
                      @click="goRecognize(item)"
                    >
                      {{ recognizeActionLabel(item) }}
                    </button>
                    <button class="action-btn" @click="goReview(item)">
                      进入核对
                    </button>
                    <button
...
<script setup>
  ...
  const batchTargetItems = computed(() =>
    onlyPendingForBatch.value
      ? selectedItems.value.filter(
          (item) => String(item?.ocr_status || '').toLowerCase() === 'pending',
        )
      : selectedItems.value,
  )
  ...
  const toggleSelectAllOnPage = (event) => {
    ...
    const baseIds = onlyPendingForBatch.value
      ? pendingPageIds.value
  ...
  const batchRecognizeSelected = async () => {
    ...
    const targetIds = batchTargetItems.value
      .map((item) => Number(item.id))
      .filter((id) => Number.isFinite(id))
    const results = await Promise.allSettled(
      targetIds.map((id) => ocrCheckerApi.startRecognize(id)),
    )
    ...
    if (successCount > 0 && failedCount === 0) {
      ...
      ElMessage.warning(
        `批量识别部分成功：成功 ${successCount}，失败 ${failedCount}（${firstErrorMessage || '请稍后重试'}）`,
      )
    } else {
  ...
  const batchDeleteSelected = async () => {
    ...
    const confirmed = window.confirm(
      `确认批量删除已勾选的 ${targetIds.length} 个文件吗？`,
    )
    if (!confirmed) return
    ...
    const results = await Promise.allSettled(
      targetIds.map((id) => ocrCheckerApi.deleteFile(id)),
    )
    ...
    if (successCount > 0 && failedCount === 0) {
      ...
      ElMessage.warning(
        `批量删除部分成功：成功 ${successCount}，失败 ${failedCount}（${firstErrorMessage || '请稍后重试'}）`,
      )
    } else {
    ...
    const nextPage =
      currentPage.value > 1 &&
      successCount > 0 &&
      successCount >= currentPageFileCount
        ? currentPage.value - 1
        : currentPage.value
  ...
  const jumpToPage = () => {
    const targetPage = Number.parseInt(
      String(pageJumpInput.value || '').trim(),
      10,
    )
  ...
  const removeFile = async (item) => {
    ...
    try {
      ...
      await fetchFiles(
        files.value.length === 1 && currentPage.value > 1
          ? currentPage.value - 1
          : currentPage.value,
      )
    } catch (error) {
...
<style scoped>
  ...
  .status-hint.review-processing {
    color: #1d4ed8;
    background: #dbeafe;
    border: 1px solid #bfdbfe;
  }
  .status-hint.reviewed {
    color: #166534;
    background: #dcfce7;
    border: 1px solid #bbf7d0;
  }
  .status-hint.to-review {
    color: #b45309;
    background: #ffedd5;
