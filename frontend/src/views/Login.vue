    ...
    <div class="login2-card" :class="{ 'login-view-style': isLogin }">
      ...
      <form @submit.prevent="handleSubmit" class="login2-form">
        ...
        <template v-else>
          <div class="form-group">
            ...
            <input
              ...
              required
            />
            <p v-if="showNicknameError" class="error-hint">{{ nicknameError }}</p>
          </div>
          ...
          <div class="form-group">
            ...
            <input
              ...
              required
            />
            <p v-if="showPasswordError" class="error-hint">{{ passwordError }}</p>
          </div>
          ...
          <div class="form-group">
            ...
            <input
              ...
              required
            />
            <p v-if="showConfirmPasswordError" class="error-hint">{{ confirmPasswordError }}</p>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="input-with-action">
              <input
                ...
                required
              />
              <button class="code-btn" type="button" @click="getVerificationCode">获取验证码</button>
            </div>
          </div>
        </template>
        ...
        <label class="terms">
          ...
          <span>我同意
            <a href="#" class="link" @click.prevent="showTerms">《IBOX用户协议》</a>
            与
      ...
      <div class="footer" :class="{ 'register-footer': !isLogin }">
        <template v-if="isLogin">
          <a href="#" class="link" @click.prevent="switchToRegister">立即注册</a>
        </template>
        <template v-else>
          <span class="have-account">我已有账户，现在</span>
          <a href="#" class="link" @click.prevent="switchToLogin">登录&gt;&gt;</a>
        </template>
      </div>
    </div>
  </div>
...
const passwordError = computed(() => {
  ...
  if (!hasNumber || !hasLetter || !hasSpecial) return '密码必须包含数字、英文和特殊字符'
  return ''
})
...
const showConfirmPasswordError = computed(() => touched.confirmPassword && !!confirmPasswordError.value)

const canSubmit = computed(() => {
  if (isLogin.value) {
    return !!formData.username.trim() && !!formData.password.trim() && formData.agreeTerms
  }
...
const handleSubmit = () => {
  ...
  const userData = {
    username: isLogin.value ? formData.username : formData.nickname,
...
.brand-img { height: 36px; width: auto; } /* 稍微增大品牌图片 */

.login2-form { display: flex; flex-direction: column; gap: 24px; } /* 增加表单项间距 */
.form-group { display: flex; flex-direction: column; gap: 10px; }
.label { color: #374151; font-size: 16px; } /* 增大字体 */
.input {
  width: 100%;
...
.code-btn:hover { background: #bae6fd; }
.have-account { color: #6b7280; margin-right: 6px; }
.forgot-link {
  position: absolute;
  right: 16px;
  top: 50%;
...
.link.minor { font-weight: 500; color: #60a5fa; }
.link:hover { text-decoration: underline; }

.terms { display: flex; align-items: center; gap: 8px; color: #6b7280; font-size: 14px; } /* 增大字体 */
.terms input { width: 18px; height: 18px; } /* 增大复选框 */
...
.submit-btn:disabled { background: #e5e7eb; border-color: #e5e7eb; color: #9ca3af; cursor: not-allowed; }
.submit-btn:not(:disabled):hover { background: #60a5fa; border-color: #60a5fa; }

.footer { text-align: center; }
.register-footer { margin-top: 24px; }
