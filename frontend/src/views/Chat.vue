  ...
  <div class="chat2-container">
    <div class="navigation-container">
      <div :class="['left-nav', { collapsed: isCollapsed }]">
        <div class="nav-header">
          <img src="../assets/talk page/talk@3X_03.png" class="logo" v-show="!isCollapsed" alt="Logo">
          <button @click="toggleNav" class="collapse-btn">
            <img src="../assets/talk page/talk@3X_40.png" alt="Toggle Navigation" class="collapse-icon">
          </button>
        </div>
        <div class="user-profile">
          ...
          <div class="user-details">
            ...
            <div v-else>
              <button class="login-btn-outline" @click.prevent="goLogin">{{ isCollapsed ? '登录' : '立即登录' }}</button>
              <p class="register-link" @click.prevent="goRegister">注册</p>
            </div>
          </div>
        </div>
        <ul class="menu-items">
          <li><a href="#" @click.prevent="startNewChat"><img src="../assets/talk page/talk@3X_10.png" class="menu-icon"><span class="menu-text"> 新建对话</span><span class="tooltip">新建对话</span></a></li>
          <li><a href="#"><img src="../assets/talk page/talk@3X_21.png" class="menu-icon"><span class="menu-text"> 当前对话</span><span class="tooltip">当前对话</span></a></li>
          <li><a href="#" @click.prevent="showHistory = !showHistory"><img src="../assets/talk page/talk@3X_28.png" class="menu-icon"><span class="menu-text"> 对话历史</span><span class="tooltip">对话历史</span></a></li>
          <li><a href="#"><img src="../assets/talk page/talk@3X_47.png" class="menu-icon"><span class="menu-text"> AI 智能体</span><span class="tooltip">AI 智能体</span></a></li>
          <li><a href="#"><img src="../assets/talk page/talk@3X_58.png" class="menu-icon"><span class="menu-text"> 我的任务</span><span class="tooltip">我的任务</span></a></li>
          <li><a href="#"><img src="../assets/talk page/talk@3X_63.png" class="menu-icon"><span class="menu-text"> 文档管理</span><span class="tooltip">文档管理</span></a></li>
          <li><a href="#"><img src="../assets/talk page/talk@3X_67.png" class="menu-icon"><span class="menu-text"> 知识库</span><span class="tooltip">知识库</span></a></li>
          <li><a href="#"><img src="../assets/talk page/talk@3X_71.png" class="menu-icon"><span class="menu-text"> 知识图谱</span><span class="tooltip">知识图谱</span></a></li>
        </ul>
        <div class="logout-section">
          <a href="#" class="logout-link" @click.prevent="handleLogout">
            <img src="../assets/talk page/talk-xt-@3_10.png" alt="logout" class="logout-icon" />
            注销登录
          </a>
        </div>
      </div>
      ...
      <template v-else>
        <div :class="['history-empty', { 'hidden-history': !showHistory }]">
          <div class="history-empty-content">
            <img class="history-empty-icon" src="../assets/talk page/talk@3x_09.png" alt="未登录提示" />
            <p class="history-empty-tip">登录后可查看历史对话！</p>
            <p class="history-login-link tooltip-link" @click.prevent="goLogin">
              立即登录
            </p>
            <p class="history-register tooltip-link" @click.prevent="goRegister">
              注册
            </p>
          </div>
        </div>
    ...
    <div class="main-content">
      <div class="chat-container">
        ...
        <!-- Messages Area -->
        <div v-else class="messages-area" ref="messagesArea" @scroll="onMessagesScroll">
          <div
          ...
          >
            ...
            <div v-else class="ai-message-content">
              ...
              >
                <img v-if="!message._copied" src="@/assets/talk%20page/talk@3_03.png" alt="复制" class="copy-icon" />
        ...
        <div class="input-container">
          <div class="input-wrapper" :class="{ 'has-text': newMessage.trim() !== '' }">
            ...
            <div class="input-toolbar">
              ...
              <div class="toolbar-right">
                <button
                  ...
                  :title="isLoading ? 'AI正在回复，无法发送' : (newMessage.trim() === '' ? '请输入内容后再发送' : '发送')"
                >
...
export default {
  ...
  setup() {
    ...
    const sendMessage = async () => {
      ...
      if (text !== '') {
        ...
        try {
                 ...
                 try {
                   ...
                   if (data.content) {
                     ...
                     if (aiMessageIndex === null) {
                       messages.value.push({ content: data.content, sender: 'ai' });
          ...
          messages.value.push({ content: '抱歉，AI 服务暂时不可用，请稍后重试。', sender: 'ai' });
          isLoading.value = false;
        }
        // 每次交互后保存当前会话状态
        saveToStorage();
    ...
    const loadFromStorage = () => {
      try {
        ...
        headerText.value = typeof data.headerText === 'string' ? data.headerText : '新的对话';
    ...
    const loadHistoryFromStorage = () => {
      try {
        ...
        if (Array.isArray(chatHistory.value) && chatHistory.value.length > HISTORY_MAX_LENGTH) {
          // 仅保留最新的 N 条（列表头部为最新）
    ...
    const saveHistoryToStorage = () => {
      try {
        ...
        localStorage.setItem(getHistoryStorageKey(), JSON.stringify(chatHistory.value));
      } catch (err) {
    ...
    const saveCurrentChat = () => {
      ...
      const titleCandidate = headerText.value || (messages.value[0]?.content?.substring(0, 20) + '...' || '新对话');
      ...
      let existingIndex = chatHistory.value.findIndex((h) => {
        return !!conversationIdCandidate && !!h.conversation_id && h.conversation_id === conversationIdCandidate;
      });

      // 若找不到，但当前有选中的历史索引，则沿用该索引进行更新（避免在会话ID生成后重复创建新项）
      if (existingIndex === -1 && currentChatIndex.value >= 0 && currentChatIndex.value < chatHistory.value.length) {
        existingIndex = currentChatIndex.value;
      }

      // 如果已有记录，则沿用其 timestamp；否则使用当前时间
      const fixedTimestamp = existingIndex !== -1
    ...
    onMounted(() => {
      ...
      const q = typeof route?.query?.prompt === 'string' ? route.query.prompt.trim() : '';
      if (q) {
        // 从首页进入时，先新建一个会话
        startNewChat();
        newMessage.value = q;
  ...
  methods: {
    ...
    async fetchUserData() {
      try {
        const raw = localStorage.getItem('ai-chat-user');
        if (raw) {
          const data = JSON.parse(raw);
