<template>
  <div class="pdf-viewer">
    <div class="toolbar">
      <div class="toolbar-left">
        <el-button
          ...
          >独立窗口预览</el-button
        >
        <el-button size="small" :loading="loading" @click="loadPdf(true)"
          >刷新预览</el-button
        >
        <el-button
          size="small"
        ...
        <el-button
          ...
          >放大</el-button
        >
        <el-button size="small" :disabled="loading" @click="downloadPdf"
          >下载</el-button
        >
      </div>
      <div class="toolbar-right">
        <span v-if="errorText" class="status">{{ errorText }}</span>
        <span v-else-if="highlightSummary" class="highlight-summary">{{
          highlightSummary
        }}</span>
    ...
    <!-- 高亮词列表面板 -->
    <div
      v-if="normalizedHighlightTerms.length && pages.length"
      class="highlight-terms-panel"
    >
      <div class="panel-header">
        <span class="panel-title"
          >已识别的高亮词 ({{ normalizedHighlightTerms.length }})</span
        >
    ...
    <div ref="scrollRef" class="content">
      <div v-if="pages.length" class="pages">
        <section v-for="page in pages" :key="page.pageNumber" class="page-card">
          ...
          >
            <canvas
              :ref="bindCanvasRef(page.pageNumber)"
              class="page-canvas"
            ></canvas>
            <div
      ...
      <el-empty
        v-else
        :description="loading ? '正在加载 PDF...' : '暂无可预览 PDF'"
      />
    </div>
  </div>
</template>
...
<script setup>
  ...
  const findHighlightRanges = (text) => {
    ...
    normalizedHighlightTerms.value.forEach((term) => {
      ...
      while (startIndex < lowerSource.length) {
        ...
        const overlapped = ranges.some(
          (item) => !(endAt <= item.start || foundAt >= item.end),
        )
        if (!overlapped) {
  ...
  const loadPdf = async (reloadBinary = false) => {
    ...
    try {
      ...
      ElMessage.warning(
        error?.response?.data?.message || error?.message || 'PDF 预览加载失败',
      )
    } finally {
      loading.value = false
    }
  ...
  const openInNewTab = async () => {
    try {
      // 用 preview=false 拉取原始 PDF 二进制，与下载接口保持一致
      const rawBlob = await ocrCheckerApi.downloadFileBlob(props.fileId, false)
      const url = URL.createObjectURL(rawBlob)

      // 新窗口内直接写入 HTML，embed 读取同域 blob URL，无跨 Tab 访问限制
      const newWin = window.open('', '_blank')
      if (!newWin) {
        URL.revokeObjectURL(url)
        ElMessage.warning('浏览器阻止了弹窗，请在地址栏右侧允许弹窗后重试')
        return
      }
...
<head>
  <meta charset="utf-8" />
  <title>PDF 预览 — ${props.fileId}</title>
...
<body>
  ...
  <script>
    // 页面卸载时释放 blob URL
    window.addEventListener('unload', function(){ URL.revokeObjectURL('${url}') })
  <\/script>
</body>
</html>`)
      newWin.document.close()
    } catch (err) {
      ElMessage.warning(err?.message || '打开新标签失败，请重试')
    }
  }
...
<style scoped>
  ...
  .pdf-highlight {
    ...
    border-radius: 3px;
    box-shadow:
      0 0 0 1.5px rgba(234, 179, 8, 0.5),
