...
try:
    ...
    _spec = importlib.util.spec_from_file_location('commission_config_settings', str(_settings_path))
    if _spec is None or _spec.loader is None:
        raise
    _settings_module = importlib.util.module_from_spec(_spec)
    _spec.loader.exec_module(_settings_module)
    DEVELOPMENT_CONFIG = _settings_module.DEVELOPMENT_CONFIG
...
try:
    ...
    _pipeline_path = project_root / 'core' / 'pipeline.py'
    _spec = importlib.util.spec_from_file_location('commission_core_pipeline', str(_pipeline_path))
    if _spec is None or _spec.loader is None:
        raise
    _pipeline_module = importlib.util.module_from_spec(_spec)
    _spec.loader.exec_module(_pipeline_module)
    OCRPipeline = _pipeline_module.OCRPipeline
