...
def get_commission_dify_config():
    enabled = str(getattr(settings, 'OCR_COMMISSION_DIRECT_DIFY_ENABLED', False)).lower() == 'true'

    base_url = (
        getattr(settings, 'OCR_COMMISSION_DIFY_BASE_URL', None)
        or getattr(settings, 'DIFY_API_URL', '')
        or ''
    ).strip().rstrip('/')

    api_key = (
        getattr(settings, 'OCR_COMMISSION_DIFY_API_KEY', None)
        or getattr(settings, 'DIFY_API_KEY_requestTesting', None)
        or ''
    ).strip()
    ...
    return {
        ...
        'base_url': base_url,
        'api_key': api_key,
        'default_user': getattr(settings, 'OCR_COMMISSION_DIFY_DEFAULT_USER', 'bowuchn@163.com'),
        'upload_endpoint': getattr(settings, 'OCR_COMMISSION_DIFY_UPLOAD_ENDPOINT', '/files/upload'),
        'workflow_endpoint': getattr(settings, 'OCR_COMMISSION_DIFY_WORKFLOW_ENDPOINT', '/workflows/run'),
        'response_mode': getattr(settings, 'OCR_COMMISSION_DIFY_RESPONSE_MODE', 'blocking'),
        'transfer_method': getattr(settings, 'OCR_COMMISSION_DIFY_TRANSFER_METHOD', 'local_file'),
        'file_type': getattr(settings, 'OCR_COMMISSION_DIFY_FILE_TYPE', 'document'),
        'timeout': float(getattr(settings, 'OCR_COMMISSION_DIFY_TIMEOUT', get_timeout())),
    }
