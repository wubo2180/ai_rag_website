...
class TextRecognitionStep(V3BaseStep):
    ...
    def _get_ocr_engine(self) -> 'PaddleOCR':
        ...
        if self._ocr_engine is None:
            ...
            # PaddleOCR 3.x 参数；如果运行环境只有 PaddleOCR 2.x，则自动回退。
            ocr_params = {
            ...
            legacy_params = {
                'use_angle_cls': self.processing_params['use_textline_orientation'],
                'lang': self.processing_params['lang'],
                'use_gpu': self.processing_params['device'] != 'cpu',
                    'ir_optim': False,
                    'enable_mkldnn': False,
                    'cpu_threads': 2,
            ...
            if not hasattr(PaddleOCR, 'predict'):
                self._ocr_engine = PaddleOCR(**legacy_params)
                self._ocr_api_mode = 'ocr'
            else:
                try:
                    self._ocr_engine = PaddleOCR(**ocr_params)
                    self._ocr_api_mode = 'predict'
                except TypeError:
                    self._ocr_engine = PaddleOCR(**legacy_params)
                    self._ocr_api_mode = 'ocr'
    ...
    def _perform_ocr_recognition(self, image_path: str, ocr_engine: 'PaddleOCR') -> Tuple[List[TextBlock], Dict[str, Any]]:
        ...
        if getattr(self, '_ocr_api_mode', 'predict') == 'ocr' or not hasattr(ocr_engine, 'predict'):
            result = ocr_engine.ocr(
                image_path,
                cls=self.processing_params['use_textline_orientation'],
            )
            ocr_data = self._normalize_legacy_ocr_result(result)
        else:
    ...
    @staticmethod
    def _normalize_legacy_ocr_result(result: Any) -> Dict[str, Any]:
        """Normalize PaddleOCR 2.x output to the PaddleOCR 3.x-like shape used downstream."""
        if not result:
            return {}
        ...
        for item in items:
            ...
            rec_scores.append(float(score or 0.0))
